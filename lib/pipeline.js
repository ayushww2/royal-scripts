...
${royalMode.isRoyalMaker(scriptMaker) ? royalMode.writeAddendum(title, targetWords) : "Royal Family Special Storytelling Mode is OFF for this Script Maker."}

Write the complete script now.`;
}

function lengthPrompt({ script, title, targetWords, actual, scriptMaker }) {
...
${royal ? `Add new REAL royal facts (dates, palaces, titles, public events, ages, funerals, marriages) AND new emotional invented quotes and private scenes that serve the title.
Keep roughly 50% real facts and 50% fictionalized title-serving emotion.
Keep it extremely emotional.` : `Add new researched detail, meaning, and retention beats. Do not invent fake quotes as fact.`}
...
async function fitWordCount({ script, title, targetWords, scriptMaker, onUpdate }) {
  let text = script;
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    for await (const token of openai.streamCompletion({
      ...
      messages: [
        ...
        { role: "user", content: lengthPrompt({ script: text, title, targetWords, actual, scriptMaker }) },
      ],
    })) {
      draft += token;
      const now = Date.now();
