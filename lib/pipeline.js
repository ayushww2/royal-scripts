...
outline: array of {id, heading, purpose, newInformation, emotionalPurpose, titleConnection, rehook},
...
async function* generateScript({ title, targetWordCount, scriptMaker, customInstructions, onStatus }) {
  ...
  const emit = (stage, label) => onStatus?.({ stage, label });
  ...
  const script = segmentScript(draft);
  ...
  yield { type: "done", saved: history.get(record.id) };
}
