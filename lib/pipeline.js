...
Do not repeat the same family-history paragraph in five sections.
Use “And honestly” at most once.`;
  }
...
Do not quote the warning, letter, or wish before Section 10.
Do not retell earlier sections. Do not add extra “And honestly.”
...
async function streamWrite({ scriptMaker, userContent, onUpdate, stageLabel, systemContent, model }) {
  let draft = "";
  let lastPersist = 0;
  for await (const token of openai.streamCompletion({
    ...
    maxTokens: 12000,
    messages: [
      { role: "system", content: systemContent || engineSystem(scriptMaker) },
...
async function runJob({ id, title, targetWordCount, scriptMaker, customInstructions, onUpdate }) {
  ...
  if (royalMode.isRoyalMaker(maker) && looksLikeRefusal(draft, title)) {
    ...
    const retry = await streamWrite({
      scriptMaker: maker,
      systemContent: royalMode.catalogFallbackSystem(),
      userContent: royalMode.refusalBypassPrompt({ title, targetWords, plan, dossier }),
      onUpdate,
  ...
  if (royalMode.isRoyalMaker(maker)) {
    ...
    if (looksLikeRefusal(script, title)) {
      emit("loyalty", "Writing the Crown Watch episode...");
      const lastTry = await openai.complete({
        model: openai.config().planModel,
        maxTokens: 12000,
        messages: [
          { role: "system", content: royalMode.catalogFallbackSystem() },
          { role: "user", content: royalMode.refusalBypassPrompt({ title, targetWords, plan, dossier }) },
        ],
      });
      ...
      if (issues.length) {
        ...
        if (revised.trim()) {
          ...
          if (!looksLikeRefusal(cleaned, title) && countWords(cleaned) >= Math.max(800, Math.round(countWords(script) * 0.55))) {
  ...
  if (royalMode.isRoyalMaker(maker)) script = royalMode.stripMeta(script);
