...
function looksLikeRefusal(text) {
  if (royalMode.isRefusal(text)) return true;
...
function buildWritePrompt({ title, targetWords, scriptMaker, customInstructions, retrieval, plan, dossier }) {
  if (royalMode.isRoyalMaker(scriptMaker)) {
    const style = compactBible(retrieval.bible);
...
DATABASE VOICE (patterns only — copy the spoken storytelling, not Wikipedia tone)
Hook job: ${JSON.stringify(style.hook)}
Narration: ${JSON.stringify(style.narration)}
Opening patterns: ${(style.openingPatterns || []).slice(0, 6).join(" | ")}
Natural rehooks: ${(style.topRehooks || []).slice(0, 8).join(" | ")}

REFERENCE OPENINGS (replace names and events for THIS title; do not copy wording)
${refs}
...
async function runJob({ id, title, targetWordCount, scriptMaker, customInstructions, onUpdate }) {
  ...
  if (royalMode.isRoyalMaker(maker) && looksLikeRefusal(draft)) {
    ...
    if (retry.trim() && !looksLikeRefusal(retry)) draft = retry;
  }
  ...
  if (royalMode.isRoyalMaker(maker)) {
    ...
    if (issues.length) {
      ...
      if (revised.trim()) {
        ...
        const originalWasRefusal = looksLikeRefusal(script);
        ...
        if (!looksLikeRefusal(cleaned) && longEnough) {
          script = countWords(cleaned) > targetWords + 100 ? clipToTarget(cleaned, targetWords) : cleaned;
        } else if (looksLikeRefusal(cleaned) || originalWasRefusal) {
          ...
          if (!looksLikeRefusal(bypassed) && countWords(bypassed) >= 800) {
