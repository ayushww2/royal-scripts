...
async function runJob({ id, title, targetWordCount, scriptMaker, customInstructions, onUpdate }) {
  ...
  if (royalMode.isRoyalMaker(maker)) {
    const issues = royalMode.qualityIssues(script);
    if (issues.length) {
      emit("loyalty", "Revising Royal Family story so it stays inside the title...");
      const revised = await openai.complete({
        model: openai.config().model,
        maxTokens: 12000,
        messages: [
          { role: "system", content: royalMode.engineSystem() },
          { role: "user", content: royalMode.revisionPrompt(script, issues) },
        ],
      });
