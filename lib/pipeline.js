...
async function planScript({ title, targetWords, scriptMaker, customInstructions, retrieval, dossier }) {
  ...
  const payload = {
    ...
    fact_dossier: royalMode.isRoyalMaker(scriptMaker) ? dossier || {} : undefined,
...
ROYAL SPECIAL MODE: ${royalMode.isRoyalMaker(scriptMaker) ? "ON — documented facts are the skeleton; invented title emotion sits on top. Use the fact dossier. Do not plan a debunk." : "OFF — do not use Royal Family special storytelling."}
${royalMode.isRoyalMaker(scriptMaker) && dossier ? `\nFACT DOSSIER:\n${JSON.stringify(dossier).slice(0, 12000)}` : ""}
...
TITLE: ${title}
TARGET WORDS: ${targetWords}
HARD WORD COUNT: land between ${targetWords - 100} and ${targetWords + 100} spoken narration words. Headings do not count. Do not stop early. Do not pad with repetition.
...
${royalMode.isRoyalMaker(scriptMaker) ? royalMode.writeAddendum(title, targetWords, dossier) : "Royal Family Special Storytelling Mode is OFF for this Script Maker."}

Write the complete script now.`;
}

function lengthPrompt({ script, title, targetWords, actual, scriptMaker, dossier }) {
...
${royal ? `Add unused REAL royal facts from public record (dates, palaces, titles, marriages, funerals, Charles, Camilla, Elizabeth, Harry, Anne, William, Diana) AND new emotional invented quotes that serve the title.
Place a documented fact beside every invented beat.
...
async function fitWordCount({ script, title, targetWords, scriptMaker, onUpdate, dossier }) {
  let text = script;
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    for await (const token of openai.streamCompletion({
      ...
      messages: [
        ...
        { role: "user", content: lengthPrompt({ script: text, title, targetWords, actual, scriptMaker, dossier }) },
      ],
    })) {
      draft += token;
      const now = Date.now();
    ...
    if (!draft.trim() || looksLikeRefusal(draft)) break;
    text = segmentScript(royalMode.isRoyalMaker(scriptMaker) ? royalMode.stripMeta(draft) : draft);
  }
  return text;
}
...
async function runJob({ id, title, targetWordCount, scriptMaker, customInstructions, onUpdate }) {
  ...
  if (royalMode.isRoyalMaker(maker)) {
    emit("research", "Researching royal people, dates, and public record...");
  ...
  if (royalMode.isRoyalMaker(maker)) {
    ...
    if (issues.length) {
      ...
      if (revised.trim()) {
        ...
        if (!looksLikeRefusal(cleaned) && countWords(cleaned) >= Math.max(800, Math.round(countWords(script) * 0.55))) {
          script = cleaned;
        } else {
