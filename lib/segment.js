...
function segmentScript(script) {
  ...
  for (const line of out) {
    ...
    if (merged.length && words(line).length < 5 && merged[merged.length - 1] && !isHeading(merged[merged.length - 1])) {
  ...
  for (const line of merged) {
    ...
    if (prev && !isHeading(prev)) {
      ...
      if (["but", "and", "while", "yet", "so"].includes(lastWord) && firstWord === lastWord) {
        cleaned[cleaned.length - 1] = prev.replace(new RegExp(`\\s+${lastWord}$`, "i"), "");
      }
    }
    cleaned.push(text);
  }
