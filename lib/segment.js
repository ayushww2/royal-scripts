...
const MIN_LINE = 12;
const MAX_LINE = 15;
const JOIN_RE = /^(and|but|because|while|when|then|so|or)$/i;

function naturalCut(list, max = MAX_LINE) {
  const end = Math.min(max, list.length) - 1;
  for (let j = end; j >= 8; j--) {
    if (/[.,;:!?]$|[”"']$/.test(list[j])) return j + 1;
  }
  for (let j = end; j >= 8; j--) {
    if (JOIN_RE.test(list[j])) return j;
  }
  return 0;
}
...
function packWordList(list) {
  ...
  for (let i = 0; i < w.length; i++) {
    ...
    if (buf.length >= MIN_LINE && buf.length <= MAX_LINE && punct) {
      ...
      const cut = naturalCut(buf);
      if (cut > 0 && cut < buf.length) {
        lines.push(buf.slice(0, cut).join(" "));
        buf = buf.slice(cut);
      } else {
        pushBuf();
      }
  ...
  if (buf.length) {
    if (buf.length >= MIN_LINE || !lines.length) {
      ...
      if (all.length <= MAX_LINE) {
        lines.push(all.join(" "));
      } else {
        const cut = naturalCut(all) || Math.min(MAX_LINE, Math.max(8, all.length - MIN_LINE));
...
function mergeShortLines(lines, preservedTitle) {
  ...
  for (const line of lines) {
    ...
    if (words(line).length < MIN_LINE && prev && !keep(prev) && prev !== "") {
      const combined = `${prev} ${line}`.trim();
      if (words(combined).length <= MAX_LINE) {
        out[out.length - 1] = combined;
        continue;
      }
    }
...
function segmentScript(script) {
  ...
  for (const line of mergeShortLines(out, preservedTitle)) {
    ...
    cleaned.push(next.text.replace(/^([a-z])/, (ch) => ch.toUpperCase()));
  }
