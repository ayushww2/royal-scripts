...
SECTION FUNCTIONS
1 THE ROYAL MOMENT THAT CHANGED EVERYTHING — present event, people, date/place, first action
2 WHAT HAPPENED INSIDE THE ROYAL TIMELINE — sequence leading to the title event
3 THE FIRST PROOF BEHIND THE TITLE — first major evidence (letter, statement, appearance, duty change, timeline)
4 THE FAMILY WOUND BENEATH THE MOMENT — only the past that gives this event emotional power
5 THE PALACE PRESSURE NO ONE COULD IGNORE — duty, succession, public expectation colliding with loyalty
6 THE ACTION THAT REVEALED THE DEEPER CONFLICT — the action that proved this was more than quiet disagreement
7 WHAT THE ROYAL EVIDENCE REVEALS — combined meaning; title event becoming unavoidable
8 THE PALACE OBSTACLE THAT COULD NOT STOP IT — strongest obstacle; it must NOT defeat the title
9 THE DECISION THAT MADE THE TITLE INEVITABLE — assemble the chain; payoff feels earned but not fully delivered
10 THE EXACT ROYAL TITLE PAYOFF — literal title event, emotional meaning, final question
...
function normalizePlan(plan, title, checklist = []) {
  ...
  const defaults = [
    ...
    "WHAT HAPPENED INSIDE THE ROYAL TIMELINE",
    "THE FIRST PROOF BEHIND THE TITLE",
    "THE FAMILY WOUND BENEATH THE MOMENT",
    "THE PALACE PRESSURE NO ONE COULD IGNORE",
    "THE ACTION THAT REVEALED THE DEEPER CONFLICT",
    "WHAT THE ROYAL EVIDENCE REVEALS",
    "THE PALACE OBSTACLE THAT COULD NOT STOP IT",
    "THE DECISION THAT MADE THE TITLE INEVITABLE",
    "THE EXACT ROYAL TITLE PAYOFF",
  ];
  const sections = [];
  for (let i = 0; i < 10; i++) {
...
function formatWriterOutline(plan) {
  ...
  const lines = [
    "DOCUMENTARY HOOK OUTLINE",
    "Research Used:",
    ...asList(hook.researchUsed).map((x) => `- ${x}`),
    "Hook Talking Points:",
  ...
  for (const s of plan.sections || []) {
    ...
    lines.push("Research Used:");
    lines.push(...asList(s.researchUsed).map((x) => `- ${x}`));
    lines.push("Talking Points:");
    lines.push(...asList(s.talkingPoints).map((x) => `- ${x}`));
    if (s.quote) lines.push(`Quote: ${s.quote}`);
    if (s.newQuestion) lines.push(`New Question Created: ${s.newQuestion}`);
    if (s.suspenseBridge) lines.push(`Suspense Bridge: ${s.suspenseBridge}`);
  }
  const l = plan.loyaltyCheck || {};
  if (Object.keys(l).length) {
    lines.push("");
    lines.push("ROYAL TITLE LOYALTY CHECK");
    lines.push(`Promised event: ${l.promisedEvent || ""}`);
    lines.push(`Main subject: ${l.mainSubject || ""}`);
    lines.push(`Emotional wound: ${l.emotionalWound || ""}`);
    lines.push(`Strongest evidence: ${l.strongestEvidence || ""}`);
    lines.push(`Literal payoff: ${l.literalPayoff || ""}`);
    lines.push(`Emotional meaning: ${l.emotionalMeaning || ""}`);
    lines.push(`Question into Section 10: ${l.questionIntoSection10 || ""}`);
    lines.push(`Viewer satisfaction: ${l.viewerSatisfaction || ""}`);
    if (l.excludedBackground) lines.push(`Do not drift into: ${l.excludedBackground}`);
  }
  return lines.join("\n");
}
...
You are not writing an encyclopedia, a Wikipedia page, a news recap, a family tree, or a list of birth dates.
You already have a title-loyal outline. Follow it in order. Do not invent a different structure.
...
FORMAT
Line 1 after TITLE: HOOK
Then the hook narration of about 130–170 words.
Then SECTION 1 through SECTION 10 using the outline headings.
The hook is not Section 1.
Then OUTRO.
...
Never turn the script into a debate about whether the title is true.
Section 10 must deliver the exact event the title promised.

EMOTION
Emotion comes from the royal event: a mother’s memory, a son protecting her, a promise, a tribute, an anniversary, an inheritance, a child caught between adults, a marriage under palace pressure, two brothers divided, a future king choosing, palace silence after pain.
...
No the title is misleading, there is no evidence, the claim falls apart, the truth may be less dramatic.
No In this dramatized scene. No reconstruction. No we can imagine. No according to reports as a habit.
...
Follow the outline below in order. Do not add extra sections. Do not skip the hook.
The hook is separate from Section 1. There are exactly 10 sections.
Write like the trained database: spoken, title-first, emotional, not a fact dump.
...
Each section must answer one question, add new information, and move toward the payoff.
...
OUTLINE THE WRITER MUST FOLLOW
${formatWriterOutline(plan) || "(follow title loyalty: hook plus 10 sections)"}`;
}
...
function qualityIssues(script, title = "", targetWords = 3500) {
  ...
  if (!/^HOOK\b/im.test(text) || !/^SECTION 1\b/im.test(text) || !/^SECTION 10\b/im.test(text)) {
    issues.push("Missing HOOK or SECTION 1–10 structure.");
  }
...
function revisionPrompt(script, issues, title, targetWords = 3500) {
  const target = Number(targetWords) || 3500;
  return `Revise this Royal Family documentary. Keep the HOOK plus SECTION 1–10.
...
- Sound like the trained database: spoken storytelling, not encyclopedia dates of birth.
- Follow HOOK, then SECTION 1 through SECTION 10, then OUTRO.
- Do not add extra biographies. Do not recite dates of birth.
- Use public quotes from the outline. Do not invent private palace dialogue.
- Do not weaken the title. Do not debunk it. Do not use meta-language.
