...
Place a public-record beat beside every private spoken line: the 1997 funeral, Diana, Our Mother in 2017, the Kensington statue on 1 July 2021, a palace statement, a public tribute.
Write Anne’s message and William’s tears as narration, not as a debate.
...
function qualityIssues(script, title = "", targetWords = 3500) {
  ...
  if (isRefusal(text, title) || opposesTitle(text, title)) {
    issues.unshift("This draft proves the title wrong. Rewrite so the title event happens. Anne reveals the message. William cannot stop crying. Do not write that records cannot confirm it.");
  }
